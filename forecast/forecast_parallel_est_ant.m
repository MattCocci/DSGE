...
[infile,numb,num] = setInfiles(indataType,...
    spath,nburn,nstate+nant_implied+1,nshocks+nant_implied,npara,nblocks,nsim);
...
if peachflag > 0,
    ...
    peachdata(1) = peachdata(1)-q_adj*dlMA_pop(1);
    peachdata(2) = peachdata(2)-100*log(MA_pop(2));
    
    % Augment peachdata with nans for expectations
    if size(peachdata,2) < nvar
        peachdata = [peachdata,nan(1,nant)];
    end
...
while ( ftell(fid.params) < num.params ) % only if PRIO==0
    ...
    TTTsim = fread(fid.TTT,[(nstate+nant_implied+1)^2,nsim],'single')';
    RRRsim = fread(fid.RRR,[(nstate+nant_implied+1)*(nshocks+nant_implied),nsim],'single')';
    zendsim = fread(fid.zend,[(nstate+nant_implied+1),nsim],'single')';
